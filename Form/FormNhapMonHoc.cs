...
namespace PhanMemQuanLyDiemSinhVien
{
    public partial class FormNhapMonHoc : Form
    {
        ...
        {
            connectionString = "Data Source=Belphegor0813;Initial Catalog=DatabaseQLDiemSV;Integrated Security=True;Encrypt=False";
            ds = new DataSet();
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string hoc_phi = Convert.ToString(dtgrv.Rows[CurrentIndex].Cells[9].Value.ToString());
                string ghi_chu = Convert.ToString(dtgrv.Rows[CurrentIndex].Cells[10].Value.ToString());
                string UpdateStr = "Update MonHoc set ma_mh='" + ma_gv + "',diem_id='" + diem_id + "',ten_mh='" + ten_mh + "',thoi_gian='" + thoi_gian + "',dia_diem='" + dia_diem + "',so_tin_chi='" + so_tin_chi + "',so_tiet='" + so_tiet + "',so_dk='" + so_dk + "',hoc_phi='" + hoc_phi + "',ghi_chu='" + ghi_chu + "' where ma_mh='" + ma_mh + "'";
                SqlCommand UpdateCmd = new SqlCommand(UpdateStr, conn);
                UpdateCmd.CommandType = CommandType.Text;
                UpdateCmd.ExecuteNonQuery();
                LoadData();
                MessageBox.Show("Bạn đã sửa thành công!!", "Thông báo", MessageBoxButtons.OK);
                conn.Close();
            }
            catch(SqlException ex)
            {
        ...
        {
            DialogResult = MessageBox.Show("Bạn có muốn thoát!", "Thông báo!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            Close(); 
        }
        ...
        {
            string searchText = tbx_TimKiem.Text.Trim();

            if (!string.IsNullOrEmpty(searchText))
            {
                DataView dv = ds.Tables["MonHoc"].DefaultView;
                dv.RowFilter = $"ma_mh LIKE '%{searchText}%' OR ten_mh LIKE '%{searchText}%'";

                dtgrv.DataSource = dv.ToTable();
            }
            else
            {
                dtgrv.DataSource = ds.Tables["MonHoc"];
